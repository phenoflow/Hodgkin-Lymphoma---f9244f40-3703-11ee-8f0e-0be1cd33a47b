...
(async () => {
  ...
  await fs.writeFile("hodgkin-lymphoma-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("hodgkin-lymphoma-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
